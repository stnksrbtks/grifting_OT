...
\section{Thinking about mean-field controls}
...
\section{Thinking about the transient distribution.}
...
\begin{align}
    \phi_v=\mu_0-q_v+ (\Id+ v)\pushforward \phi_v, 
\end{align}
...
or, reordered,
\begin{align}
    \Lc(v,\phi,\lambda_1,\lambda_2,\lambda_3,\lambda_4)=&\langle c\vert\phi\rangle+\langle \lambda_1\vert (\Id-(\Id+v)\pushforward) q_v\rangle+\langle \lambda_2\vert -(\Id+v)\pushforward \phi_v\rangle+\langle \lambda_2\vert \phi_v-\mu_0+q_v\rangle\notag\\
    &+\langle \lambda_3\vert q_v-\nu\rangle+\langle \lambda_4\vert \phi_v-\phi\rangle+\lambda_5(\phi(\Omega))\,,\label{eq: lagrangian with measures}
\end{align}
which is linear in $\phi$ and $q_v$, but non-linear in $v$.

The gradients with respect to the multipliers and $\phi$ are easy provided knowledge of $\phi_v$ and $q_v$ (except for $\lambda_3$ which requires the target $\nu$ but it can take SGD). On the other hand, taking the gradient in the control space yields
\begin{align}
...
\begin{align}
    \De_h (O_v m_v)=\bar\De_h O_v m_v+O_v \De_h m_v.
\end{align}
...
\begin{align}
    ...
    &\qquad+\langle \lambda_1 \vert \bar\De_h P_v q_v+ P_v \De_h q_v\rangle -\langle \lambda_2 \vert \bar\De_h P_v \phi_v+ P_v \De_h \phi_v\rangle.
\end{align}
...
\begin{lemma}\label{lemma: derivative of q_v}
    Let $q_v$ be the stationary distribution associated with the operator $P_v$. Then, for any $h\in\Sc$ and $v\in\Sc$, the Gateaux derivative of $q_v$ in direction $h$ is given by
    \begin{align}
        ?
    \end{align}
\end{lemma}
...
It remains to compute the Gateaux derivatives $\De_h q_v$ and $\De_h \phi_v$. To do so, we start from first principles:
\begin{align}
 q_{v+\varepsilon h}= \lim_{t\to\infty} \frac1t \sum_{s=0}^{t-1} P_{v+\varepsilon h}^s \mu_0, \qquad
\end{align}

Meanwhile, 
\begin{align}
    \phi_{v+\varepsilon h}(A) = \Eb\left[\sum_{t=1}^{+\infty} \1_{X_{t}^v\in A} - q_v(A) \right]
\end{align}


\paragraph{GPT output}
The appropriate notion of derivative here is not a pointwise derivative of a measure, but rather the weak directional derivative of the map $v\mapsto m_v$ with values in signed measures, tested against the Lagrange multipliers. Concretely, if $m_v$ stands for either $q_v$ or $\phi_v$, we say that $m_v$ is directionally differentiable at $v$ in direction $h\in\Sc$ if there exists a signed measure $\dot m_v[h]\in\Ms$ such that, for every bounded measurable test function $f$,
\begin{align}
    \frac{\de}{\de\varepsilon}\Big\vert_{\varepsilon=0}\langle f\vert m_{v+\varepsilon h}\rangle=\langle f\vert \dot m_v[h]\rangle.
\end{align}
If $h\mapsto \dot m_v[h]$ is linear and continuous, this is the Fr\'echet derivative of the measure-valued map $v\mapsto m_v$. In particular, for the functional $J(v):=\langle f\vert m_v\rangle$, one has
\begin{align}
    DJ(v)[h]=\langle f\vert Dm_v[h]\rangle.
\end{align}

Formally differentiating the fixed-point equations satisfied by $q_v$ and $\phi_v$ then gives the linearised equations
\begin{align}
    (\Id-P_v)\dot q_v[h]&=(DP_v[h])q_v,\label{eq: linearised stationary}\\
    (\Id-P_v)\dot \phi_v[h]&=-\dot q_v[h]+(DP_v[h])\phi_v.\label{eq: linearised transient}
\end{align}
The derivative of the pushforward operator is again understood weakly: for any test function $f$,
\begin{align}
    \langle f\vert (DP_v[h])\mu\rangle=\frac{\de}{\de\varepsilon}\Big\vert_{\varepsilon=0}\langle f\vert P_{v+\varepsilon h}\mu\rangle.
\end{align}
If $f$ is differentiable, this becomes
\begin{align}
    \langle f\vert (DP_v[h])\mu\rangle=\int \nabla f(x+v(x))\cdot h(x)\,\mu(\de x).
\end{align}
Hence the object that needs to be computed in the gradient of the Lagrangian is the weak Fr\'echet derivative of the signed-measure-valued maps $v\mapsto q_v$ and $v\mapsto \phi_v$, rather than a Lions derivative. The main analytical difficulty is then to justify existence of $Dq_v$ and $D\phi_v$, which typically requires stability or mixing assumptions ensuring that the resolvent $(\Id-P_v)^{-1}$ is well defined on the relevant subspace of signed measures.
